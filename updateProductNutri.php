...
//	$product -> save();
	//$upc = $product->getSku();
	$sku = trim($_REQUEST['curp']);
	$curl = curl_init('https://api.itemmaster.com/v2/item/?upc='.$sku);
	
	//$productupc = $product->getSku();
	...
	if($resultStatus['http_code'] == 200) {
		...
		exit;*/
		if($upc != ''){
		$_newProduct = Mage::getModel('catalog/product')->loadByAttribute('sku',$sku);
		...
		echo "Your SKU is not matched with item master UPC Code<br>";
	}
