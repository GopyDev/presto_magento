...
Checkout.prototype = {
    ...
    reloadProgressBlock: function(toStep) {
        var updater = new Ajax.Updater('checkout-progress-wrapper', this.progressUrl, {
			...
			onSuccess: function(){update_visited_section();},
...
//            }
    ...
    */
    nextStep: function(transport){
		...
		//set Progress
		if(response.goto_section == 'shipping_method'){//move to shipping method section
			setProgress('p_shipping_method');
		}else if(response.goto_section == 'shipping'){//move to shipping address
			setProgress('p_shipping');
		}
...
function setProgress(id){
	//alert(id);
	rel = jQuery('#'+id).attr('rel');
	
	jQuery('#opc_checkout').removeAttr('class');
	jQuery('#opc_checkout').addClass('opc-progress');
	jQuery('#opc_checkout').addClass(rel);
	/*visited = false;
	jQuery('#opc_checkout li').removeClass('opc-visited');
	jQuery.each(jQuery('#opc_checkout li'), function(i, p){
		if(!visited){
			jQuery(p).addClass('opc-visited');
			jQuery(p).removeClass('opc-waiting');
		}
		if(jQuery(p).hasClass('opc-checkout-active')){
			visited = true;
			jQuery(p).removeClass('opc-checkout-active');
			jQuery(p).removeClass('opc-waiting');
			jQuery(p).addClass('opc-visited');
		}
	});
	jQuery('#'+id).addClass('opc-checkout-active');
	jQuery('#'+id).removeClass('opc-waiting');*/
}
function backClicked(){
	window.setTimeout(function(){
		jQuery.each(jQuery('#checkoutSteps li.section'), function(i, p){
			if(jQuery(p).hasClass('active')){
				cl = jQuery(p).attr('id');
				jQuery('#opc_checkout').removeAttr('class');
				jQuery('#opc_checkout').addClass('opc-progress');
				jQuery('#opc_checkout').addClass(cl);
			}														  	
		});
	}, 2000);
}