<?php
      if(isset($_GET["gt"]) && $_GET["gt"]=='dpick' ) {
	...
	Dry Pick List :-
	 ...
	 ?>
	     <a href="<?php echo Mage::getBaseUrl(); ?>?gt=dpick&picker2=<?php echo $order_picker["picker_id"]; ?>" style="font-weight:bold;font-size:16px !important;"><?php echo $picker_name; ?></a>&nbsp;&nbsp;
	 <?php
	 }
	 ?>
	 </div>
	 <?php }
	  if(isset($_GET["picker2"]) && $_GET["picker2"]!="" && $_GET["gt"]=='dpick' ) {
	  ?>
	...
	<?php
	$sql3e = "SELECT mei.entity_id,msi.item_id,msi.product_id,mei.value,msi.product_id,msi.qty_ordered as qotd,msi.name,msi.sku,msi.`order_id`,mop.picker_id,rr.value as val,rr2.value as sval,rr3.value as tval,rr4.value as uval,rr2.store_id,
    ( CASE rr2.value
	WHEN 'Produce' THEN 1
	WHEN 'Dairy' THEN 2
	WHEN '1' THEN 3
	...
	?>
	 ...
	 <table border=1 cellpadding="5" cellspacing="5" style="border-collapse:collapse;font-size:13px;width:100%;"  >
				  <tr><td colspan=2 align="center" style="font-size:15px;color:green;"><b> Dry Pick List ( <?php echo $picker_name; ?> ) </b></td></tr>
