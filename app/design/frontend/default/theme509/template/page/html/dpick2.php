<?php
      if(isset($_GET["gt"]) && $_GET["gt"]=='dpick2' ) {
	...
	Dry Pick List :-
	 ...
	 ?>
	     <a href="<?php echo Mage::getBaseUrl(); ?>?gt=dpick2&picker2=<?php echo $order_picker["picker_id"]; ?>"><?php echo $picker_name; ?></a>&nbsp;&nbsp;
	 <?php
	 }
	 ?>
	 </div>
	 <?php }
	  if(isset($_GET["picker2"]) && $_GET["picker2"]!="" && $_GET["gt"]=='dpick2' ) {
	  ?>
    ...
    <form method="get" action="<?php echo Mage::getBaseUrl(); ?>" style="margin-top:9px;margin-bottom:9px;">
	  ...
	  $sel_location="SELECT rr2.value as sval FROM `mg_sales_flat_order_item` msi
						...
						where msw.`window` LIKE '%".$_GET["ode"]."' and mop.picker_id='".$_GET["picker2"]."' and ( mei.value=143 )  group by rr2.value";
	...
	<?php
	$sql3e = "SELECT msi.item_id,msi.product_id,mei.value,msi.product_id,msi.qty_ordered as qotd,msi.name,msi.sku,msi.`order_id`,mop.picker_id,rr.value as val,rr2.value as sval,rr3.value as tval,rr4.value as uval,rr2.store_id,
    ( CASE rr2.value
	WHEN 'Produce' THEN 1
	WHEN 'Dairy' THEN 2
	WHEN '1' THEN 3
	...
	END ) as abc,mog.`increment_id` FROM `mg_sales_flat_order_item` msi
						...
						where msw.`window` LIKE '%".$_GET["ode"]."' and mop.picker_id='".$_GET["picker2"]."' and ( mei.value=143 )";

						 if(isset($_GET["bylocation"]) && $_GET["bylocation"]!="" )
						 {
						   $sql3e.=" and rr2.value='".$_GET["bylocation"]."'";
						 }

						$sql3e.=" group by msi.item_id order by abc,rr3.value,rr4.value,msi.name";
	...
	?>
	 ...
	 <table border=1 cellpadding="5" cellspacing="5" style="border-collapse:collapse;font-size:11px;"  >
				  <tr><td colspan=6 align="center" style="font-size:15px;color:green;"><b> Dry Pick List ( <?php echo $picker_name; ?> ) </b></td></tr>
				   <tr>
                   ...
                   <?php
					...
					// $order4444 = Mage::getModel("sales/order")->load($value3e["order_id"]);

					$sel_customer_fname="select customer_firstname from mg_sales_flat_order where entity_id='".$value3e["order_id"]."' limit 0,1";
					 $customer_fname=$connection->fetchOne($sel_customer_fname);

                    $shipping_address["firstname"] = $customer_fname;


					$sel_customer_fname="select customer_lastname from mg_sales_flat_order where entity_id='".$value3e["order_id"]."' limit 0,1";
					 $customer_fname=$connection->fetchOne($sel_customer_fname);
			        ...
			        ?>
					<td style="width:430px; <?php if($qrt["0"]==2) { ?> font-weight:bold;" <?php } ?> <?php if($qrt["0"]>=3) { ?> font-weight:bold;font-style:italic;" <?php } ?> ">
		   ...
		   <a href="<?php echo Mage::getBaseUrl(); ?>?item_id=<?php echo $value3e["item_id"]; ?>&rpc=<?php echo $_GET["ode"]; ?>" style="text-decoration:underline;color:red;" target="_blank"> Product Change </a>
					...
					<td style="width:25px;font-size:15px; <?php if($qrt["0"]>=2) { ?> background:yellow;color:green;padding:3px;font-weight:bold; <?php } ?> "><?php echo $qrt["0"]; ?>
					</td><td style="width:225px;padding-right:5px;"><?php echo str_replace("'","",$shipping_address["firstname"]); ?> <?php echo str_replace("'","",$shipping_address["lastname"]); ?>
