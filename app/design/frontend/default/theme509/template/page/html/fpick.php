 <?php
      if(isset($_GET["gt"]) && $_GET["gt"]=='fpick' ) {
	...
	Frozen Pick List :-
	 ...
	 ?>
	     <a href="<?php echo Mage::getBaseUrl(); ?>?gt=fpick&picker2=<?php echo $order_picker["picker_id"]; ?>" style="font-weight:bold;font-size:16px !important;"><?php echo $picker_name; ?></a>&nbsp;&nbsp;
	 ...
	 <?php }
	  if(isset($_GET["picker2"]) && $_GET["picker2"]!="" && $_GET["gt"]=='fpick' ){
	  ?>
	...
	END ) as abc FROM `mg_sales_flat_order_item` msi
	    ...
	    <table border=1 cellpadding="5" cellspacing="5" style="border-collapse:collapse;page-break-before:always;font-size:14px;width:100%" >
				  <tr <?php if($qrt["0"]==2) { ?> style="background:#e0e0e0;font-weight:bold;" <?php } ?> <?php if($qrt["0"]>=3) { ?> style="background:#e0e0e0;font-weight:bold;font-style:italic;" <?php } ?> ><td  align="center">
                  <b>Frozen Pick List ( <?php echo $picker_name; ?> ) </b></td></tr>
					...
					$qrt = explode(".",$value3e["qotd"]);
					 if($value3e["sval"]!=$kbr)
					{
					   echo "<tr id='location-".$value3e["sval"]."' ><td colspan='3' style='text-align:center;font-size:15px;font-weight:bold;color:#008000;'> Location : ".$value3e["sval"]." ( <a href='javascript:void(0);' onClick='backdrppick();' > Go to Top </a> ) </td></tr>";
					}
					$kbr = $value3e["sval"];
