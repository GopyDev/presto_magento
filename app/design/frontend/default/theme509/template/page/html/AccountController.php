...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    public function loginPostAction()
    {
        ...
        if(isset($_POST['success_url']) && !strstr($_POST['success_url'], 'login') && !$no_redirect){
			$this->_redirectUrl($_POST['success_url']);
        }else{
    ...
    public function createPostAction()
    {
		...
		$postcode =	$_POST['postcode'];
	 	$zips = Mage::helper('simpleupc')->zipcodes();
		...
		$connection->insert('mg_zipcode', $fields);
		$connection->commit();
			$session->addError($this->__('Thank you for your interest, but we are not currently delivering to your area. Please stay tuned on our Facebook page and the FAQ page for any changes to our service area!')); 
			...
			$customer->setAcceptSubstitutes(0);
			if ($this->getRequest()->getParam('accept_substitutes', false)) {
                $customer->setAcceptSubstitutes(1);
            }
			...
			//**************End Group Assign From Code xoom***********************//
			
			//$check_code = $this->getRequest()->getPost('group_code');
			$params = $this->getRequest()->getParams();
			//$customer->setGroupId($this->getRequest()->getPost('group_code'));
			
				
			//Mage::log($params['group_code'],null,'groupcode.log');
			//Mage::log($params['group_id'],null,'groupname.log');
			
			$groupcode = $params['group_code'];
			if($groupcode == 'GEprestfresh'){
			//$params['group_id'] = '4';
          ...
          // $groupcode = substr($groupcode, 0, 2);						
			if ( $groupcode == $group['label'])
			{	  
				$grp_id = $group['value'];
				$groupcode = $grp_id;
				//Mage::log($grp_id,null,'grp_id.log');				 
				 $customer->setGroupId($grp_id);
			}
...
//**************End Group Assign From Code xoom***********************//
 ...
 //$customer ->setOccupation($this->getRequest()->getPost('occupation')); //xoom
            ...
            if ($this->getRequest()->getPost('create_address')) {
                ...
                if ($addressErrors === true) {
                    $address->setId(null)
                        ...
                        ->setIsDefaultShipping($this->getRequest()->getParam('default_shipping', false)); //Remove comment xoom
 ...
 /* Commented out.  This code changes the value of $successUrl to the home page - xoom
    ...
    public function editPostAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $customerData = $customerForm->extractData($this->getRequest());
				// Mage::log("Query product: ".print_r($customerData, true),null,'customerData.log');
			$customer->setAcceptSubstitutes(0);
			if ($this->getRequest()->getParam('accept_substitutes', false)) {
                $customer->setAcceptSubstitutes(1);
            }
            ...
            if ($customerErrors !== true) {
				...
				//If Creditcard information was requested then 
				 if ($this->getRequest()->getParam('change_ccdinfo')) {
					// Mage::log($customerData['customer_id'],null,'custd.log');
					$date = date('m/d/Y');
					$con  = mysql_connect('localhost','prestofr_staging','staging123');
					$sqlconnect = mysql_select_db('prestofr_newstaging',$con) or die(mysql_error()); 
					//Customer creditcard Information Save
						...
						//Fetch customer customer information
							...
							$table = $resource->getTableName('save_creditcarddetail');
							 
							$query = 'SELECT id FROM ' . $table . ' WHERE customer_id = '.(int)$customer_id .' LIMIT 1';
							 
							$queryresult = $readConnection->fetchOne($query);
	...
	public function questionAction()
    {
        ...
        $how_do_your_hear = $this->getRequest()->getParam('how_do_your_hear', false);
		$other_specify = $this->getRequest()->getParam('other_specify', false); 
		try {
                $customer = Mage::getModel('customer/customer')->load($this->_getSession()->getId());
                if ((!$customer) || (!$customer->getId())) {
                    echo ('Failed to load customer by id.');
					return;
                }
				$customer->setHowDoYourHear($how_do_your_hear);
				if($how_do_your_hear == 6){
					$customer->setOtherSpecify($other_specify);
				}
				$customer->save();
				echo 'Data has been save';
            }
            catch (Exception $e) {
                echo $this->__('Wrong customer account specified.');
				return;
            }
    }
		/**** END HERE FOR THE POPUPDATA SAVE********************/
}
