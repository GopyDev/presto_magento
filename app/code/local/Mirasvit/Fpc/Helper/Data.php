    ...
    public function getNormalizedUrl($protocol = false)
    {
        ...
        if ($uri) {
            ...
            foreach ($this->_getIgnoredUrlParams() as $param) {
                if (strpos($param, '=') !== false //ability create the same cache for specific param, for example p=1
                    && ($preparedParam = explode('=', $param))
                    && isset($preparedParam[0])
                    && isset($preparedParam[1])
                    && isset($query[$preparedParam[0]])
                    && $query[$preparedParam[0]] == $preparedParam[1]) {
                        unset($query[$preparedParam[0]]);
                } elseif (isset($query[$param])) {
                    unset($query[$param]);
                }
            }
            ksort($query);
