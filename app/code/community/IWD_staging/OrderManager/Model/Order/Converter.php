...
class IWD_OrderManager_Model_Order_Converter extends Mage_Core_Model_Abstract
{
    ...
    /*public function newQuoteConverter(){
        ...
        $quote->setData('store_id', $order->getStoreId())->setIsActive(0)->save();
        ...
        if(!empty($items)){
            $this->removeAllQuoteItems($quote);
            foreach ($items as $productId => $item) {
                $params = new Varien_Object($item);
                $quote_item = $this->addProductToQuote($quote, $productId, $params);
                $quote_item->save();
            }
            $quote->setTotalsCollectedFlag(false)->collectTotals();
        }
