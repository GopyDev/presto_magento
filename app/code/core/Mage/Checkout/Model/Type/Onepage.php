...
class Mage_Checkout_Model_Type_Onepage
{
    ...
    public function saveBilling($data, $customerAddressId)
    {
		...
		if($typeid[0] == 'ugiftcert'){
				//Nothing Doing			
		}else{ 
		if($data['use_for_shipping'] == 1){
			$zips = Mage::helper('simpleupc')->zipcodes();
			
				if(!in_array($data['postcode'], $zips)){
					return array('error' => 1,
							'message' => Mage::helper('checkout')->__('Sorry, we are not currently delivering to your area. A map of our service area can be found on our FAQ page. If you feel you received this message in error or if you would like to request delivery to be opened up to your location, please use the Contact Us link at the bottom of the page. We apologize for any inconvenience.')
						);		
				}
			}
		}
    ...
    public function saveShipping($data, $customerAddressId)
    {
        ...
        if (!empty($customerAddressId)) {
            $customerAddress = Mage::getModel('customer/address')->load($customerAddressId);
			//Mage::log(print_r($customerAddress['postcode'],true),null,'customerAddress.log');
		//Restrict for zipcode	
		$zips = Mage::helper('simpleupc')->zipcodes();
		if(!in_array($customerAddress['postcode'], $zips)){
			//Mage::log('allow',null,'null1.log');
			return array('error' => 1,
					'message' => Mage::helper('checkout')->__('Sorry, we are not currently delivering to your area. A map of our service area can be found on our FAQ page. If you feel you received this message in error or if you would like to request delivery to be opened up to your location, please use the Contact Us link at the bottom of the page. We apologize for any inconvenience.')
				);		
		}
		//Restrict for zipcode
        ...
        } else { Mage::log('2',null,'2222.log');
			//Mage::log('1',null,'22.log');
            ...
            $addressErrors  = $addressForm->validateData($addressData);
					...
					//Restrict for zipcode	
		$zips = Mage::helper('simpleupc')->zipcodes();
		if(!in_array($addressData['postcode'], $zips)){
			//Mage::log('allow',null,'null1.log');
			return array('error' => 1,
					'message' => Mage::helper('checkout')->__('Sorry, we are not currently delivering to your area. A map of our service area can be found on our FAQ page. If you feel you received this message in error or if you would like to request delivery to be opened up to your location, please use the Contact Us link at the bottom of the page. We apologize for any inconvenience.')
				);		
		...
		/*$zips = Mage::helper('simpleupc')->zipcodes();if(!in_array($data['postcode'], $zips)){
			Mage::log('allow',null,'null1.log');
			return array('error' => 1,
					'message' => Mage::helper('checkout')->__('Sorry, we are not currently delivering to your area. A map of our service area can be found on our FAQ page. If you feel you received this message in error or if you would like to request delivery to be opened up to your location, please use the Contact Us link at the bottom of the page. We apologize for any inconvenience.')
				);		
    ...
    public function saveShippingMethod($shippingMethod)
    {
		...
		// Alert for giftcard flexible
		$quote = Mage::getSingleton('checkout/cart')->getQuote();
		//Mage::log($quote->getGiftcertCode(),null,'flexibleship.log');
		if($quote->getGiftcertCode() != '' && $_POST['flexible_shipping'] == 'yes'){
			
			return array('error' =>  1, 
					'message' => Mage::helper('checkout')->__('Flexible Delivery cannot be chosen when a Gift Card is used. Would you like to continue? Continuing will remove your Flexible Delivery selection and you will need to choose a 2 hour delivery window. Thank you')
				);	
				 
		}
	 		// Alert for giftcard flexible End
