...
//            $data = $this->_filterPostData($postData);
    ...
    public function saveShippingMethodAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
			...
			$dateForGift = $_POST['delivery_window'];
			if(strpos($dateForGift, '|') !== false){
				$dateForGift = substr($dateForGift, (strpos($dateForGift, '|'))+1);
			}
			...
			$collection->addFieldToFilter('date', array('eq' => date("Y-m-d", strtotime(str_replace('_', '-', $dateForGift)))))->addFieldToFilter('status', 1);
			$gift = $collection->getFirstItem();
			if($gift->getProductId()){
				$id = $gift->getProductId(); // Replace id with your product id
				$qty = '1'; // Replace qty with your qty
				$productGift = Mage::getModel('catalog/product')->load($id);
				$carttoadd = Mage::getModel('checkout/cart');
				$carttoadd->init();
				$items = $carttoadd->getItems();
				foreach ($items as $item) {
					if ($item->getProduct()->getId() == $id || $item->getPrice() == 0) {
						$itemId = $item->getItemId();
						$carttoadd->removeItem($itemId)->save();
					}
				}
				$carttoadd->addProduct($productGift, array('qty' => $qty));
				$carttoadd->save();
				
				foreach ($carttoadd->getItems() as $item) {         
            ...
            */
			...
			exit;*/
			//if(isset($CustValue)){
			if($Custresults && $roleId != 0){
				unset($_SESSION['ship_charge']);
				
			}
			//echo "<pre>";
			//print_r( $this->getOnepage()->getQuote()->collectTotals());
			//exit;
			/****************************************
						END
			/****************************************/
			...
			if($data['remember'] == 'yes'){
				...
				$sqlconnect = mysql_select_db('prestofr_stage',$con) or die(mysql_error()); 
					//Customer creditcard Information Save
						...
						//End
							if($queryresult>0){
								$query2 = "UPDATE `mg_save_creditcarddetail` SET cc_type='$cc_type', cc_number='$cc_number', cc_exp_month='$cc_exp_month', cc_exp_year='$cc_exp_year', cc_cid='$cc_cid', date='$date', remember=1 where customer_id = '$customer_id'";
								$result1 = mysql_query($query2) or die(mysql_error());					
								}else{
								//if($customer_id != 0){
								$sql1 = "Insert into `mg_save_creditcarddetail` SET cc_type='$cc_type', cc_number='$cc_number', cc_exp_month='$cc_exp_month', cc_exp_year='$cc_exp_year', cc_cid='$cc_cid', date='$date', remember=1,customer_id = '$customer_id'";
								$result2 = mysql_query($sql1) or die(mysql_error());
								}
								//}
					//Customer creditcard Information Save
    ...
    public function saveOrderAction()
    {
        ...
        try {
			...
			$sql2eee = "INSERT INTO `sp` (`order_id`, `spt`) VALUES ('$orderid','$sp')";
			$result3ef = mysql_query($sql2eee) or die(mysql_error());
