...
/***************************************
		Default customer is subscribed
				START
/***************************************/
if(empty($results1)){
			...
			<tr>
				<td valign="top">
					...
					<p style="font-size:14px; line-height:22px; margin:0;">
						<a href="http://www.prestofreshgrocery.com/surveymanager/index/index/survey_id/1?order='.$orderNumber.'&customerId='.$customerId.'&driverId='.$driverId.'&pickerId='.$pickerId.'" >Click here to access the survey</a>
					</p>
...
related emails, please click here to <a href="http://www.prestofreshgrocery.com/unsubscribe-sruvey.php?customerId='.$customerId.'">unsubscribe</a>.
	...
	// $email = 'adhiyahiren108@gmail.com';
	...
	//if(mail('naeem@xoomsolutions.com',$sub,$message,$headers)){
		...
		$rs = mysql_query($qry);
		header("Location: http://www.prestofreshgrocery.com/?sign2=2&ooid=".$orderNumber);
...
/***************************************
	Default customer is subscribed
				END
/***************************************/
else if( !empty($results1) && $results1['subscribe'] == '1'){
	$email = $_REQUEST['customer_email'];
	$orderNumber = $_REQUEST['order_id'];
	$fname = $_REQUEST['first_name'];
	$lname = $_REQUEST['last_name'];
