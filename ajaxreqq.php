<?php
    ...
    $selected = mysql_select_db( DBDATABASE, $dbhandle) or die("Could not select examples"); //select a database to work with
     ...
     if($_GET["comp"]=="0")
	 {
	      $query = "update mg_sales_flat_order set gcomplete=0 where increment_id='".$_GET["ordernumber"]."'";
		  mysql_query($query);
	  }
	  else if($_GET["comp"]=="1")
	  {
	     $query = "update mg_sales_flat_order set gcomplete=1 where increment_id='".$_GET["ordernumber"]."'";
		  mysql_query($query);
	  }
