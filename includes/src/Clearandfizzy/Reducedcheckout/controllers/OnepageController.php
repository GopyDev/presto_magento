...
class Clearandfizzy_Reducedcheckout_OnepageController extends Mage_Checkout_OnepageController
{
	...
	public function savePaymentAction( $gotonext = true ) {
		...
		if ($this->_expireAjax()) {
			...
			Mage::log($validcode,null,'valid.log');
			Mage::log($data['method'],null,'method.log');
		// this is the default way
		...
		if($data['method'] == 'cashondelivery'){
			...
			if($_POST['answer'] == $validcode){
				if ( $this->_helper->skipPaymentMethod() == true) {
					...
					// attempt to load the next section
					if ( $gotonext == true ) {
						$data  = 'cashondelivery'.$_POST['answer'];
						Mage::log($data,null,'aaa.log');
		...
		if ($this->getRequest()->isPost()) {
			...
			if (!isset($result['error'])) {
				...
				if ($this->getOnepage()->getQuote()->isVirtual()) {
					...
					Mage::log($data['use_for_shipping'],null,'next1.log');
					// find out which section we should go to next
					$result = $this->getNextSection($result, $current = 'billing');
					...
					$result['goto_section'] = 'shipping';Mage::log($result['goto_section'],null,'nextsec.log');
