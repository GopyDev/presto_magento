...
class IWD_OrderManager_Model_Address
{
    ...
    {
        if ($this->isNeedRecalculateOrderTotalAmount())
        {
    ...
    protected function updateOrderAmounts()
    {
        if ($this->isNeedRecalculateOrderTotalAmount())
        {
            ...
            if($base_shipping_incl_tax !== null) {
                ...
                $order_edit->updateOrderPayment($order_id, $old_total_paid);
            }
    ...
    protected function isNeedRecalculateOrderTotalAmount()
    {
        if (isset($this->address['recalculate_amount']) && $this->address['recalculate_amount']) {
            ...
            foreach ($fields_which_affect_to_totals as $field) {
                if (isset($this->address[$field]) && $this->address[$field] != $this->order_address->getData($field)) {
                    return true;
                }
            }
        }
