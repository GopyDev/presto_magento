...
class Mage_Adminhtml_Block_Catalog_Product_Renderer_First extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{
		...
		public function render(Varien_Object $row)
		{
				...
				$qry = "select expiration_date from mg_rewardpoints_transaction WHERE transaction_id = '".$value."' ";
				$ans = $connection->fetchAll($qry);
				//echo "<pre>";
				//print_r($ans);
				if(  $ans[0]['expiration_date'] != '0000-00-00 00:00:00'){
					return $ans[0]['expiration_date'];
				}else{
					return '';
				}
