...
class IWD_OrderManager_Model_Order_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    {
        $selected_columns = $this->getSelectedColumnsArray();
    ...
    public function getSelectedColumnsArray()
    {
        $selected_columns = Mage::getStoreConfig(self::XML_PATH_ORDER_GRID_COLUMN);
    ...
    protected function addFiltersToCollection($collection, $filter)
    {
        ...
        //$message = Mage::helper('iwd_ordermanager')->__('Purchased period form %s to %s');
        //Mage::getSingleton('adminhtml/session')->addNotice(sprintf($message, $from, $to));
    ...
    public function getOrdersCollection($selected_columns, $collection = null)
    {
        ...
        $sales_flat_order = array('shipping_description', 'customer_email', 'coupon_code', 'weight', 'customer_note');
        ...
        if (!empty($selected_col)) {
            $collection->getSelect()->joinLeft($tableName_sales_flat_order_item,
                "main_table.entity_id = {$tableName_sales_flat_order_item}.order_id",
                array('product_sku' => new Zend_Db_Expr('group_concat(DISTINCT sku SEPARATOR ", ")'),
                    'ordered_products' => new Zend_Db_Expr('group_concat(DISTINCT name SEPARATOR ", ")'),
                    'product_options' => new Zend_Db_Expr('group_concat(DISTINCT product_options SEPARATOR "|| ")'),
    ...
    public function getOrderGridColumns()
    {
        ...
        return array(
            ...
            'order_comment' => $helper->__('Order Comment'),
