...
class Mage_Adminhtml_CustomerController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction()
    {
        ...
        if ($data) {
			...
			if($formData["group_id"]==22 || $formData["group_id"]==23 || $formData["group_id"]==24)
			{
			    $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
				
				//$query = "delete from `mg_pickers` where email='".$formData["email"]."' "; 
		        // $connection->query($query);
				 
				
				if($formData["group_id"]==23)
				{
				    $role='driver';
				}
				else
				...
				$nameb=$formData["lastname"].",".$formData["firstname"];
				
				$sel_name="select id from `mg_pickers` where email='".$formData["email"]."'";
				$name=$connection->fetchOne($sel_name);
				
				if($name=="")
				{
				  $query = "insert into `mg_pickers` set `name` = '".$nameb."',email='".$formData["email"]."',role='".$role."' "; 
		          $connection->query($query);
				}
				else
				{
				   $query5 = "update `mg_pickers` set `name` = '".$nameb."',role='".$role."' where  email='".$formData["email"]."'"; 
			...
			{
			   $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
			   $query5 = "update `mg_pickers` set extra='inactive' where  email='".$formData["email"]."'"; 
		           $connection->query($query5);
			}
