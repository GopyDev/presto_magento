 ...
 public function indexAction()
    {
        ...
        if ($cart->getQuote()->getItemsCount()) {
			...
			foreach($items as $cartItem) {
      		 ...
      		 if(!$nonNominalItems && $typeid[0] != 'ugiftcert'){
				 Mage::log('11',null,'null11.log');
			  // if($this->_getQuote()->getGiftcertCode() == '' && $this->_getQuote()->getCouponCode() == ''){
				 
				 if($this->_getQuote()->getGiftcertCode() == '' ){
				   Mage::log('11',null,'null22.log');
					if (!$this->_getQuote()->validateMinimumAmount()) {
						    Mage::log('11',null,'null3.log');
						$minimumAmount = Mage::app()->getLocale()->currency(Mage::app()->getStore()->getCurrentCurrencyCode())
							->toCurrency(Mage::getStoreConfig('sales/minimum_order/amount'));
						//echo "Minimum amount is ".$minimumAmount;
						 if(!Mage::getSingleton('customer/session')->isLoggedIn()){
							//echo "Go in not logged in condition..".
                            ...
                            ;
							$sql= "SELECT * FROM mg_aw_sarp2_profile WHERE customer_id = '".$customerData->getId()."'";
		...
		$_SESSION['flat_rate_Charge'] = $shipping_charge1;
        ...
        try {
            ...
            if (!$product) {
				...
				foreach($items as $prod){
					...
					$productval = Mage::getSingleton('catalog/product')->load($prod->getId());
