...
//	echo Mage::helper('checkout/cart')->getQuote()->getSubtotal();
	// echo "subtotal:->".$subtotal;
...
//	if($subtotal<50)
	...
	$cart = $this->_getCart();
		...
		// echo $cart;
		{//custom code - remove applied delivery window discounts
			if(Mage::getSingleton('checkout/cart')->getQuote()->getCouponCode() == 'flexible_delivery'){
				$cart->getQuote()->setCouponCode('')->collectTotals()->save();
				
				header('Location: '.Mage::getBaseUrl().'checkout/cart/');
				exit;
			}
			if(trim(Mage::getSingleton('checkout/cart')->getQuote()->getCouponCode()) == 'add_3_dollar'){
				$cart->getQuote()->setCouponCode('')->collectTotals()->save();
				
			
				
				echo Mage::getSingleton('checkout/cart')->getQuote()->getCouponCode();
				header('Location: '.Mage::getBaseUrl().'checkout/cart/');
				exit;
			}
		}
...
foreach($items as $prod){
	...
	if($prod->getProductType() != 'customproduct'){
		$product_id[] = $prod->getId();
		$product = Mage::getSingleton('catalog/product')->load($prod->getId());
	}
}

if(in_array("customproduct",$prod_type) && in_array("simple",$prod_type)){
	try{
		foreach($product_id as $item1){
			$item2 = Mage::getModel('catalog/product')->load($item1);
			Mage::dispatchEvent('catalog_controller_product_delete', array('product' => $item2 ));
			$item2->delete();
...
$nonNominalItems = Mage::helper('aw_sarp2/quote')->getAllSubscriptionItemsFromQuote($quote);
		...
		Mage::log($typeid[0],null,'types.log');
		
       if(!$nonNominalItems && $typeid[0] != 'ugiftcert'){
            if (!$this->_getQuote()->validateMinimumAmount()) {
			
                $minimumAmount = Mage::app()->getLocale()->currency(Mage::app()->getStore()->getCurrentCurrencyCode())->toCurrency(Mage::getStoreConfig('sales/minimum_order/amount'));
			//	echo $minimumAmount;
			//	exit;
				
				
	Mage::log('ugicert',null,'2222.log');
    ...
    public function addAction()
    {
        ...
        try {
			...
			if($output === false){
				...
				$session = Mage::getSingleton('checkout/session');
				$session->addError("You can't add to cart this product with subscription");
